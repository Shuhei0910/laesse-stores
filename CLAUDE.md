...
1. **シート1（代理店URL）の赤セルを確認**
   ...
   - **⚠️ 絶対ルール：赤セルを消すのは、そのセルの変更をindex.htmlに反映した後のみ。先にクリアすると修正漏れが追跡不能になる（2026-04-23 実際に発生）**
...
### スプシ↔index.html 不一致チェック（疑いがある時に実行）
赤セルを消したが修正が漏れた可能性がある場合など、スプシのB列（ニックネーム）をキーに住所を突き合わせて不一致を検出する：
```python
# シート1 B～H列取得 → name(B) + address(F+G+H) をindex.htmlのname/addressと比較
# 差分が「スペースのみ」以外の店舗が修正漏れ候補
```
スペース有無だけの差は正常（スプシは店舗名を建物名列に詰めて入力するため）。都道府県や番地が丸ごと違う店舗が真の不一致。
...
- 2026-04-24: 修正漏れ発覚→スプシ↔HTML突き合わせでシャルム（広島）の住所が別店舗データ（静岡）になっていたことを発見・修正。Realiser~レアリゼ~（愛媛）の未コミット修正もまとめてプッシュ。新規取扱店2件追加（yoli：山口県プレステージ／ヘアサロンピュア：岐阜県ベーシック）。赤セルクリア前に全修正完了を必須とするルールをCLAUDE.mdに追記
